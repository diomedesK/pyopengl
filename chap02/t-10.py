...
class Executor(Base):
    ...
    def update(self):
        ...
        #using the deltaTime (miliseconds elapsed since last screen update)
        #makes speed constant across all frame rates
        distance = self.deltaTime * SPEED 
        if self.input.isKeyPressed("up"):
            self.translation.data[1] += self.deltaTime
        if self.input.isKeyPressed("down"):
            self.translation.data[1] -= self.deltaTime
        if self.input.isKeyPressed("right"):
            self.translation.data[0] += self.deltaTime
