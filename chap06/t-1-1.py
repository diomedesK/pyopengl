...
class Executor(Base):
    ...
    def initialize(self):
        ...
        # self.pointLight = PointLight(color=[0.1, 0.0, 1.0], position=[-6, 0, 5])
        
        self.ambientLight = AmbientLight(color=[0.1, 0.1, 0.1])
        self.directionalLight = DirectionalLight(color=[0.8, 0.8, 0.8], direction=[-1, -1, -2])
        self.pointLight1 = PointLight(color=[0.9, 0, 0], position=[-120/6, 0, 0])
    ...
    def update(self):
        ...
        self.pointLight1.translate(math.sin(self.timer) / 6, 0, 0)
        print(self.pointLight1.getPosition())
